...
// change default to something without texture
 ...
 //
	...
	{	
		...
		const { resolveSize, resolveVelocity, resolveFade, resolveDrift, resolveOffset } = builder;
	 ...
	 // everything defined here happens on runtime and threejs 
		...
		{ case 'image':
		 ...
		 // auto scale image on screen
		  ...
		  case 'offset':
			if(value.length > 1)
				con.background.offset = resolveOffset( value, 'vh' );
			return;
 ...
 //	
	...
	{// apply style definitions from parser on canvas context
		const con = painter.context;
		const { resolveSize, resolveOffset, resolveBlurSize, resolveThickness, thicknessLimit } = builder;
		 ...
		 // stroke operation
		  case 'layer.stroke': // only arrays arrive here!
			...
			{	const { strings, numbers, arrays } = splitTypes( value );
				...
				arrays.forEach( ( e ) =>
				{	if( e.length == 2 )
						con.focus.stroke.offset = resolveOffset( e );
				} );
			}
			else if( value == '-' )
		  ...
		  case 'layer.stroke.offset':
			if(parseFloat(value[0]) || parseFloat(value[1]))
				con.focus.stroke.offset = resolveOffset( value );
			return;
		 ...
		 // fill operation
		  case 'layer.fill': // only arrays arrive here!
			...
			{	const { strings, numbers, arrays } = splitTypes( value );
				...
				arrays.forEach( ( e ) =>
				{	if( e.length == 2 )
						con.focus.fill.offset = resolveOffset( e );
				} );
			}
			else if( value == '-' )
		  ...
		  case 'layer.fill.offset':
			if(parseFloat(value[0]) || parseFloat(value[1]))
				con.focus.fill.offset = resolveOffset( value );
			return;
	 ...
	 // blur operation
			...
			{	const { strings, numbers, arrays } = splitTypes( value );
				...
				{	switch( i )
					{ case 0:
					  ...
					  case 1:
						if( e.length == 2 )
							con.focus.blur.offset = resolveOffset( e );
						break;
				}	} );
		  ...
		  case 'layer.blur.offset':
			if(parseFloat(value[0]) || parseFloat(value[1]))
				con.focus.blur.offset = resolveOffset( value );
			return;
	 ...
	 // layer Texture
		  ...
		  case 'layer.offset':
			if(parseFloat(value[0]) || parseFloat(value[1]))
				con.focus.layer.offset = resolveOffset( value );
			return;
